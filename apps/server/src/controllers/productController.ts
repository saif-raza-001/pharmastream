...
// ============ UPDATED: addBatch with netPrice support ============
export const addBatch = async (req: Request, res: Response) => {
  try {
    const { productId, batchNo, expiryDate, mrp, saleRate, purchaseRate, currentStock, netPrice } = req.body;
    
    // Get product to access gstRate for netPrice calculation
    const product = await prisma.product.findUnique({ where: { id: productId } });
    const gstRate = product?.gstRate || 12;
    
    // Calculate netPrice if not provided
    const finalNetPrice = netPrice || (saleRate * (1 + gstRate / 100));
...
// ============ UPDATED: importProducts with netPrice support ============
// ============ UPDATED: importProducts with last batch rates fallback ============
  ...
  try {
    ...
    for (const p of products) {
      try {
        ...
        // Try to find ACTIVE product by barcode first (include batches for rate fallback)
        if (p.barcode) {
          existing = await prisma.product.findFirst({
            where: { barcode: p.barcode, isActive: true },
            include: { batches: { where: { isActive: true }, orderBy: { createdAt: 'desc' }, take: 1 } }
          });
        }
        ...
        if (!existing) {
          ...
          existing = allProducts.find(
            (prod: any) => prod.name.toLowerCase() === p.name.toLowerCase().trim()
          );
        }

        // If still not found, check if there's an INACTIVE product we can reactivate
        if (!existing) {
          let inactiveProduct: any = null;
        ...
        // ✅ NEW: Get last batch rates as fallback
        const lastBatch = existing?.batches?.[0];
        
        // Get GST rate
        const gstRate = Number(p.gst) || Number(p.gstRate) || existing?.gstRate || 12;
        
        // ✅ UPDATED: Use Excel value → Last batch value → 0
        const saleRate = Number(p.saleRate) || Number(p.sRate) || (lastBatch ? Number(lastBatch.saleRate) : 0);
        const mrp = Number(p.mrp) || (lastBatch ? Number(lastBatch.mrp) : 0);
        const purchaseRate = Number(p.purchaseRate) || Number(p.pRate) || (lastBatch ? Number(lastBatch.purchaseRate) : 0);
        
        // ✅ UPDATED: NetPrice fallback chain: Excel → Last batch → Calculate → 0
        let netPrice: number;
        if (p.netPrice) {
          netPrice = Number(p.netPrice);
        } else if (lastBatch?.netPrice) {
          netPrice = Number(lastBatch.netPrice);
        } else if (saleRate > 0) {
          netPrice = saleRate * (1 + gstRate / 100);
        } else {
          netPrice = 0;
        }
...
// ============ UPDATED: updateBatch with netPrice support ============
export const updateBatch = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { batchNo, expiryDate, mrp, saleRate, purchaseRate, currentStock, netPrice } = req.body;
    
    // Get batch to access product for gstRate
    const existingBatch = await prisma.productBatch.findUnique({
      where: { id },
      include: { product: true }
    });
    
    const gstRate = existingBatch?.product?.gstRate || 12;
